...
//
// $Id: texture_glut.cpp 5352 2014-11-11 08:33:45Z mshafae $
//
...
void initUniforms_B( ){
  uLight2_position = glGetUniformLocation(shaderProgram_B->id( ),
                                          "light2_position");
  uLight3_position = glGetUniformLocation(shaderProgram_B->id( ),                                                         "light3_position");
}
...
void init() {
  ...
  // Blinn-Phong
  const char* vertexShaderSource_A = "data/blinn_phong.vert.glsl";
  const char* fragmentShaderSource_A = "data/blinn_phong.frag.glsl";
  ...
  shaderProgram_A->texture = new Texture2D("data/granite_128.png");
  printf("Shader program A built from %s and %s.\n",
         vertexShaderSource_A, fragmentShaderSource_A);
  shaderProgram_A->deactivate( );

  // Gouraud Shader
  const char* vertexShaderSource_B = "data/gouraud.vert";
  const char* fragmentShaderSource_B = "data/gouraud.frag";
  ...
  shaderProgram_B->texture = new Texture2D("data/owenlake_128.png");
...
void displayCallback( ){
  ...
  // The teapot on the right (brownish)
  shaderProgram_A->activate( );
  shaderProgram_A->texture->bind( );
  msglError( );
  activateUniforms_A( );
  glLoadIdentity( );
  ...
  // The teapot on the left (pinkish)
  shaderProgram_B->activate( );
  shaderProgram_B->texture->bind( );
  activateUniforms_B( );
  glLoadIdentity( );
  glMultMatrixf(viewingTransform);
  myTranslatef(modelingTransform,
